...
## 8. Lucide Icons
* Register every icon in `src/app/app.config.ts` before use. Check `LucideAngularModule.pick({ ... })`; if missing, import from `lucide-angular` (PascalCase) and add to `pick()`. Common: `minus`, `plus`, `x`, `trash-2`, `search`, `edit`, `pencil`, `save`, `arrow-left`, `arrow-right`, `chevron-down`, `shield-alert`, `scale`, `alert-triangle`, `tag`, `image`, `plus-circle`.
