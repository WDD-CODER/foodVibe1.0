...
describe('KitchenStateService', () => {
  ...
  let itemDataSpy: jasmine.SpyObj<ItemDataService>;
  ...
  beforeEach(() => {
    ...
    const iSpy = jasmine.createSpyObj('ItemDataService', ['addItem', 'updateItem', 'deleteItem']);
    const uSpy = jasmine.createSpyObj('UserMsgService', ['onSetSuccessMsg', 'onSetErrorMsg']);
    
    // 3. Directly assign the signal to the spy property so calling iSpy.allItems_() works
    iSpy.allItems_ = mockAllItemsSignal;

    TestBed.configureTestingModule({
      providers: [
        KitchenStateService,
        { provide: ItemDataService, useValue: iSpy },
        { provide: UserMsgService, useValue: uSpy }
      ]
    });

    service = TestBed.inject(KitchenStateService);
    itemDataSpy = TestBed.inject(ItemDataService) as jasmine.SpyObj<ItemDataService>;
    userMsgSpy = TestBed.inject(UserMsgService) as jasmine.SpyObj<UserMsgService>;
  });

  describe('Initialization & Sync', () => {
    it('should sync products_ signal when ItemDataService signal updates', () => {
      const mockRawItem = {
        _id: 'p1',
        itemName: 'Tomato',
  ...
  describe('Product CRUD', () => {
    it('should call addItem and show success message', (done) => {
      const product = createMockProduct('p2');
      itemDataSpy.addItem.and.resolveTo();

      service.addProduct(product).subscribe(() => {
        expect(itemDataSpy.addItem).toHaveBeenCalled();
    ...
    it('should delete product if exists and show success', (done) => {
      ...
      const rawItem = { _id: 'p-del', itemName: 'Delete Me', properties: {} };
      mockAllItemsSignal.set([rawItem]);
      TestBed.flushEffects(); 

      itemDataSpy.deleteItem.and.resolveTo();

      service.deleteProduct('p-del').subscribe(() => {
        expect(itemDataSpy.deleteItem).toHaveBeenCalledWith('p-del');
