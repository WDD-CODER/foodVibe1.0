...
description: Walks the live Render deployment at desktop viewport across predefined flows. Reports functional bugs: persistence failures, false validation errors, API/console errors, sync gaps. Report-only, never fixes.
---
...
- Always target the Render URL `https://foodvibe.onrender.com` — never localhost.
- **Cold-start note:** Render free tier sleeps after 15 min idle. First `$B goto` may take up to 60s — budget for this, abort only if >60s with no response.
...
4. Apply the **functional probe set** (F1-F6) as relevant to each flow:
   - **F1 — Persist round-trip:** after save, hard-reload (`$B goto <same url>`), verify data re-renders correctly. Fail if data is absent or mutated.
   - **F2 — Update-without-change:** open existing record, save without editing → must NOT fire "name already taken" / duplicate validator. Targets the recipe-update false-positive bug reported 2026-04-23.
   - **F3 — Logout/login continuity:** logout → login → data still present (Atlas sync verification).
   - **F4 — Soft-delete correctness:** delete item → verify removed from list → verify appears in `/trash` → verify restore works.
   - **F5 — API error capture:** after every save/delete, run `$B network` — flag any 4xx/5xx responses (excluding expected 404s like recipe→dish resolver fallback noted in codebase).
...
### 4. Severity Rubric
- **Critical:** Flow cannot complete — API 500, false validation block, data loss on reload, auth failure, infinite spinner, data gone after logout/login.
- **Major:** Flow completes but wrong result — stale list after save, sync gap between recipes+dishes, cross-type duplicate false positive, missing items after reload.
- **Minor:** Console warning, cosmetic desktop glitch incidental to the flow (desktop layout bugs are out of scope — those go through mobile-flow-auditor at desktop viewport in a separate command).
...
- Never retry on auth-state-mutating failures — a failed save is evidence, don't mask it by retrying.
