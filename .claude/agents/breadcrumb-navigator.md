...
Apply all project standards from `.claude/copilot-instructions.md`. Folder structure and major seam definitions: see **§4**.
...
## When to Invoke
- New routed area (`pages/<name>/`) or new top-level folder under `src/app/`
...
- Keep at **major seams** only (`core/`, `core/services`, `core/models`, `core/components`, `shared/`, `pages/`) — not every leaf folder.
...
### 4. Integration Gate
- All agents should read `breadcrumbs.md` before modifying any directory.
- After significant structural changes, trigger this agent to update affected breadcrumbs.
- Directory priorities per `copilot-instructions.md §4`: `core/services` → `core/models` → `pages/` → `shared/` → `core/` → root config.
