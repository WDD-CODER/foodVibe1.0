---
name: techdebt
description: Scans for duplicated code, dead code, style violations, and TODO debt â€” run before PRs, after features, or at session end. Maintains a rolling archive of the last 7 audit reports.
---
...
**Style Violation Rules (inline â€” no guide read required):**
- Flag: `@Input`/`@Output` decorators â†’ replace with `input()`, `output()`, `model()`
- Flag: `BehaviorSubject` â†’ replace with `signal()`
- Flag: `any` types â†’ replace with explicit types
- Flag: semicolons in TypeScript files
- Flag: components or services exceeding 300 lines â†’ refactor candidate
- Flag: temporary auth bypasses or hardcoded keys â†’ security risk, must fix before PR
- Reusable logic â†’ move to `shared/` or `core/utils/`

---

## Report Archive â€” Rolling 7-Report Retention
...
**Retention logic â€” execute at the START of every audit, before writing the new report:**
...
3. Count existing reports:
   - **Count < 7:** No deletion needed â€” proceed to write the new report.
   - **Count â‰¥ 7:** Delete the oldest file(s) until only 6 remain, making room for the new report (result: 7 after write).
4. Write (or overwrite) today's report: `techdebt-YYYY-MM-DD.md`.

> **Edge case:** If the audit runs multiple times on the same day, the existing report for that date is overwritten in-place â€” it does NOT count as a new addition, so no old reports are deleted.
...
- **Fast-path skip:** If no `.ts` files are in scope â†’ skip entirely and report clean
...
## Phase 1: Static Analysis `[Procedural â€” Haiku/Composer (Fast/Flash)]`
...
**TODO Audit:** Scan for `// TODO` or `// FIXME` comments â€” categorize by urgency (critical / nice-to-have).
...
**Refactor Candidates:** Identify components or services exceeding 300 lines â€” propose split strategy.

**Signal Optimization:** Identify imperative logic convertible to declarative Signals or `computed()` values.

**Security Surface:** Verify no temporary auth bypasses or hardcoded keys remain â€” these are blocking, must be resolved before PR.

---

## Phase 3: Report Generation & Archive Management

**3a â€” Retention cleanup:**
```
...
4. If today's date already has a report â†’ that slot will be overwritten (no deletion needed unless count > 7).
5. If count of *other* dates â‰¥ 7 â†’ delete oldest until 6 remain.
6. Write today's report.
```

**3b â€” Report template:** Each report file must follow this structure:

```markdown
# Tech Debt Audit â€” YYYY-MM-DD
...
<!-- Compare today's totals vs previous reports in the folder.
     Show direction arrows: â†‘ worse / â†“ better / â†’ stable -->
```
...
## Phase 4: Documentation & Sync `[Procedural â€” Haiku/Composer (Fast/Flash)]`

**Breadcrumb Check:** Run `update-docs` skill to ensure navigation maps reflect the cleaned state.

**Ledger Update:** Mark completed items in `.claude/todo.md` â€” move unresolved debt to a dedicated "Tech Debt" section.
...
If critical logic was changed â†’ invoke QA Engineer for verification before committing.
