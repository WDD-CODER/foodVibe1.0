...
- **Commit / push to GitHub** `[SHARED]`: User says "commit", "push", "commit to github", or uses `/commit-to-github` → read `.claude/skills/commit-to-github/SKILL.md` and follow all phases in order. No git writes until user approves the visual tree in chat. **Argument shortcut**: `/commit-to-github c` or `commit c` goes directly to Path [C] (checkpoint); `/commit-to-github s` or `commit s` goes directly to Path [S] (ship). **Do NOT trigger for general "save" or file update requests.**
