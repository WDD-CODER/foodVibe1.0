...
Invoke the Render flow audit persona to run the full render-flow audit against the live deployment.
...
- `--include-gemini` â€” include the `menu-intelligence-event` flow (default: skipped to protect Gemini quota)
...
1. Probe Render URL with cold-start tolerance:
   ...
   # if $code != 200 after 60s â†’ abort "Render service unreachable (cold start exceeded 60s or service down)"
   ```
...
5. Ensure `.gitignore` contains `.claude/reports/render-audit/.credentials.json` â€” if not, add it.
...
| 7 | `recipe-builder-edit` | `/recipe-builder/edit/:id` | Yes | Open existing prep recipe, add 5 more ingredients, reorder steps, change yield unit, save. **F2 is MANDATORY â€” must reproduce or confirm fixed the duplicate-name-on-edit bug reported 2026-04-23.** | F1, F2, F5, F6 |
...
**Probe legend:**
F1 = Persist round-trip (save â†’ hard-reload â†’ verify) | F2 = Update-without-change (open â†’ save unchanged â†’ no false validator) | F3 = Logout/login continuity | F4 = Soft-delete correctness (delete â†’ trash â†’ restore) | F5 = API error capture (network log after every save/delete) | F6 = Console error capture (console log after every state change)
...
```
You are the Render flow audit (this command file (agent retired — protocol is inline below / above)).
...
8. Write .claude/reports/render-audit/<slug>/report.md (overwrite) with:
   ...
   - Defect entries: ### [severity] <element> â€” <summary>
...
|------|----------|----------|-------|-------|----------|--------------|--------|
| signup | 2026-04-24 | 0 | 0 | 1 | 0 | 0 | [â†’](./signup/report.md) |
| ... | ... | ... | ... | ... | ... | ... | ... |
```

## Step 6 â€” Final summary

Print the Render flow audit's standard output block (from persona file).
