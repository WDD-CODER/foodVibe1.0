...
* **API**: Use `input()`, `output()`, `model()` â€” no `@Input`/`@Output`.
...
* **Utils**: Put shared helpers in `src/app/core/services/util.service.ts` (or `core/utils/`); no one-off helpers in components. Utilities must be pure (same inputs â†’ same outputs; no I/O or mutation of arguments/shared state).
...
* **breadcrumbs.md**: Keep maps at **major** seams (`src/app/core/`, `core/services`, `core/models`, `core/components`, `shared/`, `pages/`) â€” not in every leaf folder. If you add a new `pages/<feature>/` or top-level subtree under `src/app/`, add or refresh the nearest `breadcrumbs.md` via the `breadcrumb-navigator` skill.
...
* **Engine placement (hard rule)**: `.c-*` engine classes belong **only** in `src/styles.scss`. Never define a `.c-*` class in a component `.scss` file â€” Angular view encapsulation will scope it and break cross-component reuse.
* **Property order**: Layout â†’ Dimensions â†’ Content â†’ Structure â†’ Effects.
* **Shared UI**: Before any new UI (control, layout, or pattern), scan `src/app/shared/` and `src/styles.scss` (`.c-*` engines) for something composable. Prefer shared structures â€” add page-local markup only when nothing fits.
...
* **Unit tests**: Add or update `.spec.ts` when a unit is finalized, during `commit-to-github`, or when the user asks â€” avoid churn mid-iteration. Run only **targeted** specs for files you changed. Run the **full** suite only in `commit-to-github` Phase 0 or when the user asks.
* **Do NOT write `.spec.ts` during iterative plan execution** â€” spec authoring is for `commit-to-github` Phase 0 or explicit user request only.
