...
When adding a new focusable element to either page, update this file in the same PR:
1. Insert the control at the correct position in the numbered sequence above.
2. Renumber all entries below the insertion point.
3. Set `tabindex="-1"` on any controls that are intentionally skipped (e.g. +/- stepper buttons).
4. If the new control opens a sub-panel or dropdown, add a note describing the keyboard interaction (Enter/Space to open, Arrows to navigate, Escape to close).
