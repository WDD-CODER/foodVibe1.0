...
- [~] M7 Task 23 — Menu Intelligence mobile-logic port, partial: toolbar's 5 pills had no min-height (~26-30px, below touch floor) — added `min-height: var(--tap)`, verified all 5 now exactly 44px via gstack. Deliberately left the screen's own 600px/620px breakpoints and nested export-dropdown icon buttons for a dedicated mobile-audit pass, not a same-session guess on an 803-line stylesheet
