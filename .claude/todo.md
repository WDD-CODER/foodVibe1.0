...
## 🔴 Operational (needs manual action / PR / deploy)
...
> Code implementation (Steps 1–12) fully verified 2026-04-24. Steps 0 and 13 are operational.
...
> Sections 3–8 verified done 2026-04-24. Sections 1 and 9 still have open items.

- [ ] Section 1 — Sign-in / Sign-up: **dev user dropdown** (auto-focus, Enter, field errors already done)
- [x] Section 2 — Quick-add default base unit: set signal to 'gram'
- [x] Section 3 — Recipe view: FormatQuantityPipe, selectedUnit_, convertedYieldAmount_
- [x] Section 4 — Recipe builder: localStorage collapse state, no arrows, CounterComponent qty, CDK drag-drop
- [x] Section 5 — Maison Plus: row border/hover, CounterComponent qty, category-first add, auto-focus
- [x] Section 6 — App-wide category/unit dropdowns: `__add_category__` / `__add_unit__` sentinels
- [x] Section 7 — Logistics: fit-content chip width, logisticsHighlightedIndex_ keyboard nav
- [x] Section 8 — Add-equipment modal: single-step category creation (no ConfirmModal)
- [ ] Section 9 — Labels: recipe-builder manual label selector (toggleManualLabel) + "label already exists — select it?" flow
...
> Superseded by Plan 259 (DB-backed server-side approach). Kept for reference — do not implement.
...
## 📱 Mobile Audit Re-runs (browser QA pending)
...
| 072 | Robust login, app-wide logging, security | Done |
...
| 089 | Menu Intelligence Upgrade | Done |
...
| 167 | Category/unit add-new audit | Done |
...
| 190 | Agent process optimization / Master De-Spaghettification Map | Done |
...
| 247 | Reflect: self-improving skills system | Done |
| 248 | Transloco Migration | Planned |
| 249 | Catalog Seeder Data Quality + Supplier Model | Partial (code done; Steps 0+13 operational) |
| 255 | Dead Code Cleanup | Partial (Tasks 8–10, 16 open) |
| 256 | Gemini Few-Shot Injection (superseded) | Superseded by 259 |
| 259 | DB-Backed Shared Few-Shot Pool | Partial (Task 5 open) |
| 276 | Design System Token Gaps + Cook Mode Overlay | Planned |
| 284 | Context Monitor Token Rewrite | Partial (Tasks 4–5 open) |
