...
- Port allocation: start at `[WORKTREE_PORT]`, retry up to 5 candidates, hard stop if all occupied
...
**Port Allocation:** Find next available port starting from `[WORKTREE_PORT]`.
...
**Dependencies:** Run `[INSTALL_COMMAND]` inside the new worktree to ensure it is runnable.
