...
| Commit, push, create PR, merge | `git-agent` (`.claude/agents/git-agent.md`) only |
...
**MCP availability**: `.mcp.json` is configured with the GitHub MCP server using `${GITHUB_TOKEN}` from the environment. If `mcp__github__*` tools are not available in a session, fall back to `gh` CLI silently — do not block the workflow.
