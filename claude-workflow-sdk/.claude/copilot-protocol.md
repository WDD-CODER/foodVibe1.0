...
5. **MemPalace observations** *(when configured)* (when answering "why" or recalling decisions — if MCP tools available)
...
> **MemPalace context** *(when configured)* adds ~170 tokens for wake-up. Per-session search cost is ~2,700 tokens for 1 search (5 results). If MemPalace MCP is unavailable, skip silently — do not block on it.

---

## 0.3 Context-First Protocol (All Agents)

**Main Claude runs `mempalace_search` before reading files to understand an existing area** *(when MemPalace is configured)*.
Subagents receive MemPalace results injected into their prompts — they do NOT call MemPalace themselves.
...
**For unfamiliar modules:** Also call `mempalace_traverse(start_room="<module-name>", max_hops=2)` to discover cross-cutting connections.

**If MCP unavailable:** Skip without blocking — but report in your completion message that MemPalace was not consulted.
