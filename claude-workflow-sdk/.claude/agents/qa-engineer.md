...
> **Do NOT write or update spec files during iterative plan execution.**
...
- **E2E Mapping**: Determine critical user paths requiring end-to-end coverage.
...
- Co-locate specs with source as `<name>.spec.[ext]`.
- Only write during git-agent spec gate or explicit user request.

### 4. Visual QA (via gstack)
- Run `/preflight` before visual QA steps.
- After layout-affecting changes, run `/qa http://localhost:[DEV_PORT]/<relevant-page>` to verify rendered output.
