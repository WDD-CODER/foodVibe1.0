...
1. Check dev server: `curl -s -o /dev/null -w "%{http_code}" http://localhost:[DEV_PORT]` must return `200`. If not → abort with `"Dev server not running. Start with [DEV_COMMAND]."`
