...
- **Backend Impact check** — if the plan includes a `## Backend Impact` section with new collections, verify the new entity type key is registered in the project's entity registry before marking the task done
- **[FRAMEWORK] icon/asset gate** — after any task that touches a template or view file: grep for framework-specific icon or asset registrations added in that task and confirm they are imported in the project root config. Missing registration is a blocker — fix before marking the task complete.
...
3. If the diff is empty or the file count is significantly lower than reported:
   ...
   - Investigate each plan's target file to check if the change is pre-existing or genuinely missing
...
3. If verification PASSES and the task touched view/template/style files:
