...
The nightly audit agent (runs last) will pick up all staging files matching
`YYYY-MM-DD-*.md` and merge them into `.claude/reports/audit/YYYY-MM-DD-nightly-audit.md`.

## Current nightly agents and their slugs

> **[PLACEHOLDER]** Fill in your scheduled agents after running `/init-repo`.

| Agent | Trigger time | Slug |
|---|---|---|
| Nightly audit | *(set in CLAUDE.md RemoteTrigger)* | *(owner — merges all sections)* |
