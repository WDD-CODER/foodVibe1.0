...
**MCP availability**: Both `.mcp.json` (Claude Code) and `.cursor/mcp.json` (Cursor) are configured with the GitHub MCP server using `${GITHUB_TOKEN}` from the environment. If `mcp__github__*` tools are not available in a session, fall back to `gh` CLI silently — do not block the workflow.
