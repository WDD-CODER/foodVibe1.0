...
overview: Unify export UX with a floating header pattern; add view-before-export (paper-style preview) and "export all together" (one workbook); recipe-mode exports cooking steps (not checklist); unit prices only in shopping lists; 2 decimals everywhere; checklist by_station and two-sheet layout for menu.
todos: []

# Unified export refactor and upgrade

Refactor all exports to use a **unified export header** (floating button → hidden header with context-specific actions), add **view before export** (paper-style preview) and **export all together** (one file, multiple sheets). In recipe/dish mode, “checklist” becomes **export cooking steps** (steps + times + item info + date). Unit prices **only in shopping lists**; rounding **2 decimals** for amounts, currency, and percentages. Menu: reactive formulas, checklist by_station, two-sheet layout when not by_dish.
...
### 2.1 Trigger options (choose one or combine)


| Option                       | Description                                                                                                                                                                           | Pros                                      | Cons                                         |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------------------------------------------- |
| **A. Confirm before export** | Single “Export shopping list” (etc.); on click show dialog: “View first” / “Export now”. “View first” opens the preview; from preview user can “Export to Excel” or “Print” or close. | One button per export type; clear choice. | Extra click when user just wants to export.  |
| **B. Two actions in header** | Each export type has “View” and “Export” (e.g. “View list” + “Export to Excel”).                                                                                                      | No dialog; direct.                        | More buttons in the bar.                     |
| **C. Always preview first**  | “Export” always opens the preview; inside preview: “Export to Excel”, “Print”, “Close”. No direct download without preview.                                                           | User always sees what they get.           | Extra step for power users who export often. |
| **D. Export dropdown**       | One “Export shopping list” with dropdown: “View before export” and “Export to Excel”.                                                                                                 | Single entry point, two choices.          | Slightly more UI complexity.                 |


**Recommendation**: **A** or **D**. A keeps one button and one small dialog; D keeps one button and a dropdown. Both avoid cluttering the bar with two buttons per type (B) and avoid forcing preview every time (C).

### 2.2 Shared data for preview and file
...
- **Actions inside preview** (if using A or C): “Export to Excel”, “Print” (window.print on the preview content or a print-specific clone), “Close”.
...
## 7. Checklist vs cooking steps (recipe vs menu context)
...
- **Recipe/dish context** (cook-view or recipe-builder): The “checklist” action becomes **Export cooking steps**. Output: **only cooking steps and times** (order, instruction, time per step), with item name and relevant info + **export date**. One sheet; no prep-item checklist in recipe mode. Naming: e.g. `cooking-steps_{recipeName}_{date}.xlsx`.

## 8. Checklist: by station and two sheets when not by dish (menu only)

- **By station**: Add mode **by_station** (group by dish `default_station`_).
...
- **Recipe/dish** (cook-view or recipe-builder):  
  - **Sheet 1**: Item header and relevant info (name, type, yield, station, quantity, date, etc.).  
  - Then **one sheet per other type**: recipe/dish info (or ingredients list), shopping list, cooking steps (workflow), etc. So one workbook = header + one sheet for each export category (info, shopping list, cooking steps, etc.), same pattern as menu.
...
- **View before export**: Choose trigger (A or D recommended); shared export payload; new preview component (paper-style, popup, click-outside close); reuse menu paper styling.
- **ExportService**: View-model API for preview + file; naming; rounding (2 decimals everywhere); RTL, translations; two-sheet checklist (menu); by_station; unit prices **only in shopping lists**; **cooking steps** export in recipe/dish context (replaces checklist there); **export all together** (one workbook: menu = sheet1 menu + one sheet per type (checklist, shopping list, etc.); recipe/dish = sheet1 header + one sheet per type (info, shopping list, cooking steps, etc.)). Formulas in menu shopping list for reactivity.
- **Recipe-builder**: Form snapshot → export payload; floating export bar; “checklist” → “cooking steps” export.
- **Cook-view**: Floating export bar; pass current quantity/scaling; “checklist” → “cooking steps” export.
