...
**Why the obvious fix is wrong:** It looks safe because each popover is individually `@if`-gated on `viewExportModal_() === '<id>'`, so only one *should* be in the DOM at a time. But that makes the accessible name unique only as a side effect of that gating being correct right now — an accessible-name query itself isn't scoped to "the popover the user just opened," it's scoped to "whatever currently renders under that name." Nothing stops a future change (an animation that keeps the outgoing popover mounted a frame longer, a bug in the `@if` condition) from silently reintroducing a real duplicate, and any `/browse`/Playwright script written against "the ייצוא button" alone is already fragile today, not just in that future.

**What to do instead:** When automating this page, scope the query to the open `.view-export-modal` container for the specific wrap (e.g. via the wrap's own `aria-expanded`/label, `export_recipe_info` vs `export_shopping_list` etc.), never to the inner download button's accessible name alone. If this component grows more export options, give each `.view-export-option` its own translation key (e.g. `export_recipe_info_confirm`) instead of sharing the generic `'export'` key across all five.
